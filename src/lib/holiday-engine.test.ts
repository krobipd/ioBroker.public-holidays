...
describe("config: country/state/region", () => {
  it("country-only config returns holidays", () => {
    const result = computeHolidays(makeConfig({ country: "DE" }), ["en"], { referenceDate: makeDate("2026-01-01") });
  ...
  it("state config narrows holidays (DE/BY has Fronleichnam, DE/HH does not)", () => {
    const by = computeHolidays(makeConfig({ state: "BY" }), ["de"], { referenceDate: makeDate("2026-06-04") });
    const hh = computeHolidays(makeConfig({ state: "HH" }), ["de"], { referenceDate: makeDate("2026-06-04") });
  ...
  it("state config adds holidays (DE/BE has Frauentag Mar 8)", () => {
    const be = computeHolidays(makeConfig({ state: "BE" }), ["de"], { referenceDate: makeDate("2026-03-08") });
    const nw = computeHolidays(makeConfig({ state: "NW" }), ["de"], { referenceDate: makeDate("2026-03-08") });
  ...
  it("state config narrows holidays (IT/32 Alto Adige has Pentecost Monday, plain IT does not)", () => {
    ...
    const altoAdige = computeHolidays(makeConfig({ country: "IT", state: "32" }), ["it"], {
      referenceDate: makeDate("2026-05-25"),
    });
    const plainIt = computeHolidays(makeConfig({ country: "IT" }), ["it"], { referenceDate: makeDate("2026-05-25") });
  ...
  it("full country/state/region config works (DE/BY/A — Augsburger Friedensfest Aug 8)", () => {
    ...
    const withRegion = computeHolidays(makeConfig({ country: "DE", state: "BY", region: "A" }), ["de"], {
      referenceDate: makeDate("2026-08-08"),
    });
    const withoutRegion = computeHolidays(makeConfig({ country: "DE", state: "BY" }), ["de"], {
      referenceDate: makeDate("2026-08-08"),
    });
  ...
  it("state config narrows holidays (CH/BE has Berchtoldstag Jan 2, plain CH does not)", () => {
    const bern = computeHolidays(makeConfig({ country: "CH", state: "BE" }), ["de"], {
      referenceDate: makeDate("2026-01-02"),
    });
    const plainCh = computeHolidays(makeConfig({ country: "CH" }), ["de"], { referenceDate: makeDate("2026-01-02") });
  ...
  it("state config narrows holidays (US/CA has César Chávez Day Mar 31, plain US does not)", () => {
    const california = computeHolidays(makeConfig({ country: "US", state: "CA" }), ["en"], {
      referenceDate: makeDate("2026-03-31"),
    });
    const plainUs = computeHolidays(makeConfig({ country: "US" }), ["en"], { referenceDate: makeDate("2026-03-31") });
  ...
  it("different countries differ on same date (DE vs AT on Oct 26)", () => {
    const de = computeHolidays(makeConfig({ country: "DE" }), ["de"], { referenceDate: makeDate("2026-10-26") });
    const at = computeHolidays(makeConfig({ country: "AT" }), ["de"], { referenceDate: makeDate("2026-10-26") });
    expect(de.today.isHoliday).toBe(false);
    expect(at.today.isHoliday).toBe(true);
  });

  it("country without states works (JP)", () => {
    const result = computeHolidays(makeConfig({ country: "JP" }), ["en"], { referenceDate: makeDate("2026-01-01") });
    expect(result.today.isHoliday).toBe(true);
  });

  it("country with many holidays (IN)", () => {
    const config = makeConfig({ country: "IN", holidayTypes: ["public", "bank", "optional", "observance"] });
    const result = computeHolidays(config, ["en"], { referenceDate: makeDate("2026-01-26") });
    expect(result.today.isHoliday).toBe(true);
  });

  it("normal working day is not a holiday", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-03-11") });
...
describe("type filter", () => {
  it("filters out non-matching types", () => {
    const result = computeHolidays(makeConfig({ holidayTypes: ["bank"] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
    expect(result.today.isHoliday).toBe(false);
  });

  it("shows holidays when type matches", () => {
    const result = computeHolidays(makeConfig({ holidayTypes: ["public"] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
    expect(result.today.isHoliday).toBe(true);
  });
  ...
  it("empty type filter shows nothing", () => {
    const result = computeHolidays(makeConfig({ holidayTypes: [] }), ["de"], { referenceDate: makeDate("2026-01-01") });
    expect(result.today.isHoliday).toBe(false);
  });

  it("observance type adds observance holidays (DE/BY Aug 15)", () => {
    const without = computeHolidays(makeConfig({ state: "BY", holidayTypes: ["public"] }), ["de"], {
      referenceDate: makeDate("2026-08-15"),
    });
    const withObs = computeHolidays(makeConfig({ state: "BY", holidayTypes: ["public", "observance"] }), ["de"], {
      referenceDate: makeDate("2026-08-15"),
    });
...
describe("exclude list", () => {
  it("excludes a holiday by ID", () => {
    const before = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-01-01") });
    expect(before.today.isHoliday).toBe(true);

    const after = computeHolidays(makeConfig({ excludeHolidays: [toHolidayId("Neujahr", "01-01")] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
  ...
  it("excluded holiday does not appear in next", () => {
    const before = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-12-24") });
    expect(before.tomorrow.isHoliday).toBe(true);

    const after = computeHolidays(makeConfig({ excludeHolidays: [toHolidayId("1. Weihnachtstag", "12-25")] }), ["de"], {
  ...
  it("non-matching exclude ID has no effect", () => {
    const result = computeHolidays(makeConfig({ excludeHolidays: ["nonexistent_holiday"] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
    expect(result.today.isHoliday).toBe(true);
  });
  ...
  it("multiple holidays can be excluded", () => {
    ...
    const jan1 = computeHolidays(config, ["de"], { referenceDate: makeDate("2026-01-01") });
    const dec25 = computeHolidays(config, ["de"], { referenceDate: makeDate("2026-12-25") });
  ...
  it("excluded holiday does not generate bridge day", () => {
    ...
    const result = computeHolidays(config, ["de"], { referenceDate: makeDate("2026-05-15") });
    expect(result.today.isHoliday).toBe(false);
  });
});
...
describe("exclude warning (unmatchedExcludes)", () => {
  it("does not warn for an exclude that is only valid in a sibling state of the same country", () => {
    ...
    const result = computeHolidays(
      ...
      ["de"],
      { referenceDate: makeDate("2026-06-28") },
    );
...
describe("bridge days", () => {
  ...
  it("Saturday holiday does not create bridge day", () => {
    const config = makeConfig({ includeBridgeDays: true });
    const result = computeHolidays(config, ["de"], { referenceDate: makeDate("2021-12-31") });
    expect(result.today.isHoliday).toBe(false);
  });

  it("Wednesday holiday does not create bridge day", () => {
    const config = makeConfig({ includeBridgeDays: true });
    const result = computeHolidays(config, ["de"], { referenceDate: makeDate("2024-12-24") });
    expect(result.today.isHoliday).toBe(false);
  });
  ...
  it("detects a bridge day in the following year (year-boundary)", () => {
    ...
    const config = makeConfig({ includeBridgeDays: true });
    const result = computeHolidays(config, ["de"], { referenceDate: makeDate("2025-12-31") });
  ...
  describe("bridge day name localization", () => {
    ...
    it.each([
      ...
      const config = makeConfig({ includeBridgeDays: true });
      const result = computeHolidays(config, [lang], { referenceDate: bridgeDayDate });
      expect(result.today.name).toBe(expected);
    });

    it("unknown language falls back to English", () => {
      const config = makeConfig({ includeBridgeDays: true });
      const result = computeHolidays(config, ["xx"], { referenceDate: bridgeDayDate });
...
describe("relative days", () => {
  it("yesterday shows previous day holiday", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-01-02") });
  ...
  it("tomorrow shows next day holiday", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-12-24") });
...
describe("next holiday", () => {
  ...
  it("next holiday daysUntil is correct (1 day before)", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-04-02") });
  ...
  it("year rollover: next from December finds January", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-12-27") });
    expect(result.next.isHoliday).toBe(true);
    expect(result.next.date).toBe("2027-01-01");
  });

  it("next holiday always has ISO date format", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-06-01") });
    expect(result.next.date).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });
});
...
describe("localization", () => {
  ...
  it("language affects holiday names (same holiday, different language)", () => {
    const de = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-01-01") });
    const en = computeHolidays(makeConfig(), ["en"], { referenceDate: makeDate("2026-01-01") });
    expect(de.today.name).not.toBe(en.today.name);
  });

  it("Italian names for IT holidays", () => {
    const result = computeHolidays(makeConfig({ country: "IT" }), ["it"], { referenceDate: makeDate("2026-01-01") });
    expect(result.today.name).toContain("Capodanno");
  });

  it("French names for FR holidays", () => {
    const result = computeHolidays(makeConfig({ country: "FR" }), ["fr"], { referenceDate: makeDate("2026-01-01") });
    expect(result.today.name.length).toBeGreaterThan(0);
  });
...
describe("edge cases", () => {
  it("leap year Feb 29", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2028-02-29") });
    expect(result.today.isHoliday).toBe(false);
  });

  it("Silvester is not a public holiday in DE", () => {
    const result = computeHolidays(makeConfig(), ["de"], { referenceDate: makeDate("2026-12-31") });
    expect(result.today.isHoliday).toBe(false);
  });
  ...
  it("empty next holiday has consistent shape", () => {
    const result = computeHolidays(makeConfig({ holidayTypes: [] }), ["de"], { referenceDate: makeDate("2026-03-11") });
  ...
  it("all types enabled returns more holidays than public only", () => {
    ...
    const pubResult = computeHolidays(publicOnly, ["de"], { referenceDate: makeDate("2026-08-15") });
    const allResult = computeHolidays(allTypes, ["de"], { referenceDate: makeDate("2026-08-15") });
...
describe("country diversity", () => {
  ...
  for (const cc of countries) {
    it(`${cc}: does not crash and returns valid structure`, () => {
      ...
      const result = computeHolidays(config, ["en"], { referenceDate: makeDate("2026-01-01") });
  ...
  it("all tested countries have at least one holiday per year", () => {
    for (const cc of countries) {
      const config = makeConfig({ country: cc });
      const result = computeHolidays(config, ["en"], { referenceDate: makeDate("2026-06-15") });
      expect(result.next.isHoliday).toBe(true);
    }
  });

  it("US: Jul 4 is a holiday", () => {
    const result = computeHolidays(makeConfig({ country: "US" }), ["en"], { referenceDate: makeDate("2026-07-04") });
    expect(result.today.isHoliday).toBe(true);
  });

  it("FR: Jul 14 is a holiday", () => {
    const result = computeHolidays(makeConfig({ country: "FR" }), ["fr"], { referenceDate: makeDate("2026-07-14") });
    expect(result.today.isHoliday).toBe(true);
  });

  it("GB: Dec 25 is a holiday", () => {
    const result = computeHolidays(makeConfig({ country: "GB" }), ["en"], { referenceDate: makeDate("2026-12-25") });
    expect(result.today.isHoliday).toBe(true);
  });
  ...
  it("BR: Sep 7 is a holiday", () => {
    const result = computeHolidays(makeConfig({ country: "BR" }), ["pt"], { referenceDate: makeDate("2026-09-07") });
    expect(result.today.isHoliday).toBe(true);
  });
  ...
  it("bridge day is created for a non-DACH country (FR Ascension Thu -> Fri)", () => {
    ...
    const result = computeHolidays(config, ["fr"], { referenceDate: makeDate("2026-05-15") });
    expect(result.today.isHoliday).toBe(true);
...
describe("same-date collision priority", () => {
  it("shows the higher-priority (public) holiday when several types collide on one day", () => {
    ...
    const allTypes = computeHolidays(
      ...
      ["en"],
      { referenceDate: makeDate("2027-12-24") },
    );
    const publicOnly = computeHolidays(makeConfig({ country: "AS", holidayTypes: ["public"] }), ["en"], {
      referenceDate: makeDate("2027-12-24"),
    });
...
describe("unmatched excludes", () => {
  it("reports an exclude ID that matches no holiday in the data", () => {
    const result = computeHolidays(makeConfig({ excludeHolidays: ["definitely_not_a_real_holiday"] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
  ...
  it("does not report an exclude that matches a real holiday", () => {
    const result = computeHolidays(makeConfig({ excludeHolidays: [toHolidayId("Neujahr", "01-01")] }), ["de"], {
      referenceDate: makeDate("2026-01-01"),
    });
